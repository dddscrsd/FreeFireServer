...
// The `pairings` collection bridges the game's WebView and the browser that
// actually completes the Discord OAuth flow. Each dialog request creates a
// pending row identified by `pair_id`; the callback route updates it to
// `completed` once tokens have been issued; the WebView polls
// /pair/status?id=<pair_id> and consumes the row on success.
//
// Rows expire automatically via a TTL index on `expires_at`. The default
// lifetime is 10 minutes — long enough for a slow user, short enough that
// abandoned rows do not accumulate.

const PENDING = 'pending';
const COMPLETED = 'completed';
const FAILED = 'failed';

function createPairingsRepo(db) {
